...
class WDDP_const:
    def __init__(self, system, path=None, seed=1337):
        ...
        self.lambda_ = self.system.lambda_
        ...
        self.h = np.zeros(( self.T, self.N_samples, self.nw, 1))
        self.h_bar = np.zeros((self.T, self.nw, 1))
    ...
    def riccati(self, Phi, P, r, z, lambda_, t, x_bar, u_bar, w_bar, w_bar_hat, Sigma_hat, w_hat):
        ...
        Theta_W = self.Xi[t].T @ r + 2*lambda_*(w_bar - w_hat)
        Theta_W_bar = self.Xi[t].T @ r - 2*lambda_*(w_bar - w_bar_hat)
        Theta_XX = self.A[t].T @ P @ self.A[t] + self.Q[t]
        Theta_UU = self.B[t].T @ P @ self.B[t] + self.R[t] 
        Theta_WW = self.Xi[t].T @ P @ self.Xi[t] - lambda_*np.eye(self.nw)
        ...
        L_init = - np.linalg.solve(Theta_UU - Theta_UW @ np.linalg.solve(Theta_WW, Theta_WU), Theta_U - Theta_UW @ np.linalg.solve(Theta_WW, Theta_W_bar) )
        
   
        Theta_UU_reg = Theta_UU + self.reg_lambda*np.eye(self.nu)*(self.regType == 1)
        ...
        L, result, R, free = boxQP(Theta_UU_reg - Theta_UW @ np.linalg.solve(Theta_WW, Theta_WU), Theta_U - Theta_UW @ np.linalg.solve(Theta_WW, Theta_W_bar), lower, upper, L_init, options)
        ...
        h_sample = np.zeros((self.N_samples, self.nw, 1))
        for i in range(self.N_samples):
            h_sample[i] = - np.linalg.solve(Theta_WW_reg, Theta_W[i] + Theta_WU @ L )

       
        H = - np.linalg.solve(Theta_WW_reg, Theta_WX + Theta_WU @ K )
        h = - np.linalg.solve(Theta_WW_reg, Theta_W_bar + Theta_WU @ L )

        
        P_ = Theta_XX + K.T @ Theta_UX + Theta_XU @ K + H.T @ Theta_WX + Theta_XW @ H + K.T @ Theta_UU @ K + H.T @ Theta_WW @ H + K.T @ Theta_UW @ H + H.T @ Theta_WU @ K
        r_ = Theta_X + K.T @ Theta_U + H.T @ Theta_W_bar + Theta_XU @ L + Theta_XW @ h + K.T @ Theta_UU @ L + H.T @ Theta_WW @ h + K.T @ Theta_UW @ h + H.T @ Theta_WU @ L
        z_ = z + self.c[t] + L.T @ Theta_U + h.T @ Theta_W_bar + 0.5*(L.T @ Theta_UU @ L + h.T @ Theta_WW @ h + L.T @ Theta_UW @ h + h.T @ Theta_WU @ L) - lambda_*np.trace(Sigma_hat) - lambda_**2*np.trace(np.linalg.solve(Theta_WW, Sigma_hat))
        
        return P_, r_, z_, K, L, H, h, h_sample
    ...
    def backward(self, xuw, x_bar, u_bar, w_bar, w_bar_np, w_bar_hat, Sigma_hat, w_hat):
        ...
        while not done:
            ...
            for t in range(self.T-1, -1, -1):
                ...
                Phi = self.B[t] @ np.linalg.inv(self.R[t]) @ self.B[t].T + 1/self.lambda_ * self.Xi[t] @ self.Xi[t].T
                
                out = self.riccati(Phi, self.P[t+1], self.r[t+1], self.z[t+1], self.lambda_, t, x_bar[t], u_bar[t], w_bar_np[t], w_bar_hat[t], Sigma_hat[t], w_hat[t])
            
                if out is True:
                    diverge = True
                    break
                else:
                    self.P[t], self.r[t], self.z[t], self.K[t], self.L[t], self.H[t], self.h_bar[t], self.h[t] = out
                    diverge = False
    ...
    def simulate(self, render, outputs, sim_ind):
        ...
        w = np.zeros((self.T_sim+1, self.nw, 1))
        ...
        while not done:
            ...
            if render:
                self.system.render(message=self.path+'WDDP_const')
    ...
    def forward(self, alpha,  x, u, w):
        ...
        h_sample = torch.from_numpy(self.h)
    ...
    def run(self, sim_ind):
        ...
        xs = torch.from_numpy(xs_np).type(torch.DoubleTensor)
