...
class MinMaxDDP:
    ...
    def riccati(self, P, r, z, t, x_bar, u_bar, w_bar, w_bar_hat, Sigma_hat, w_hat):       
        ...
        Theta_UU = self.B[t].T @ P @ self.B[t] + self.R[t] + 1e-6*np.eye(self.nu)
        Theta_WW = self.Xi[t].T @ P @ self.Xi[t] + 1e-6*np.eye(self.nw)
        ...
        K = - np.linalg.solve(Theta_UU - Theta_UW @ np.linalg.solve(Theta_WW, Theta_WU), Theta_UX - Theta_UW @ np.linalg.solve(Theta_WW, Theta_WX) )
        L = - np.linalg.solve(Theta_UU - Theta_UW @ np.linalg.solve(Theta_WW, Theta_WU), Theta_U - Theta_UW @ np.linalg.solve(Theta_WW, Theta_W) )
        H = - np.linalg.solve(Theta_WW - Theta_WU @ np.linalg.solve(Theta_UU, Theta_UW), Theta_WX - Theta_WU @ np.linalg.solve(Theta_UU, Theta_UX) )
        h = - np.linalg.solve(Theta_WW - Theta_WU @ np.linalg.solve(Theta_UU, Theta_UW), Theta_W - Theta_WU @ np.linalg.solve(Theta_UU, Theta_U) )
    ...
    def backward(self, xuw, x_bar, u_bar, w_bar, w_bar_np, w_bar_hat, Sigma_hat, w_hat):
        ...
        for t in range(self.T-1, -1, -1):
            ...
            self.P[t], self.r[t], self.z[t], self.K[t], self.L[t], self.H[t], self.h[t] = self.riccati(self.P[t+1], self.r[t+1], self.z[t+1], t, x_bar[t], u_bar[t], w_bar_np[t], w_bar_hat[t], Sigma_hat[t], w_hat[t])
